...
export function PlanPage() {
  ...
  const handleSaveFunding = (val: number) => {
    updateStrategy({
      recurringFunding: { ...strategy.recurringFunding, amount: val },
    });
    updateBudget({ debtAllocationAmount: val });
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            {/* Monthly contribution with inline edit */}
            <div>
              ...
              {isEditingFunding ? (
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    handleSaveFunding(parseFloat(fundingInput) || 0);
                  }}
          ...
          {totalMonthlyIncome > 0 && (
            <p className="text-xs text-gray-500">
              {formatCurrency(totalMonthlyIncome)} income − {formatCurrency(budget.monthlyExpenses)} expenses = {' '}
              <span className={availableForDebt > 0 ? 'text-green-600 font-medium' : 'text-red-500 font-medium'}>
                {formatCurrency(availableForDebt)} available
              </span>
            </p>
          )}
        </div>
